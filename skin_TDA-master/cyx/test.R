...
# Rscript RTDA.R test1.txt output0.txt output1.txt
#load libraries
library(TDA)
library(ggplot2)
library(MASS)
...
coordinate_data_path <- "test1.txt"
output0 <- "output1.txt"
output1 <- "output2.txt"
...
# 在图像上设置网格，并使用k Nearest Neibor方法计算每个网格点的白色像素密度
Xlim <- c(0, 1400);
Ylim <- c(0, 1200);
by <- 10;
...
D_zero$density <- density(D_zero$midlife, D_zero$lifetime)
