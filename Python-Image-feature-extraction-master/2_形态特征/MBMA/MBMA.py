# Multilevel Binary Morphological Analysis多级二进制形态分析
# 与GSMA相同，但带有灰度图像。区别在于模式谱的计算。
...
for i,image_path1 in tqdm(enumerate(images_path1),total=len(images_path1)):
    ...
    for image_path2 in images_path2:
        ...
        # print(mask_one)
        # 提取特征
        pdf_L, pdf_M, pdf_H, cdf_L, cdf_M, cdf_H = pyfeats.multilevel_binary_morphology_features(image, mask_zero, N=30, thresholds=[25, 50])
        features = pdf_L.tolist()
        ...
        pyfeats.plot_pdfs_cdfs(pdf_L, pdf_M, pdf_H, cdf_L, cdf_M, cdf_H)
        break
    break
    # if i == 0:
    #     labels.append("TEWL")
...
output_excel = "GSMA_features.xls"
