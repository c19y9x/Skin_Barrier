# 在图像处理、计算机视觉和相关领域中，图像矩是图像像素强度的某个特定加权平均值（矩），或此类矩的函数，通常选择具有某些吸引人的属性或解释。 
# Zernikes矩是一种正交复矩，其核是极坐标空间中单位圆盘内部定义的一组Zernike完全正交多项式。 
# Zernike 矩是： 1-25) 关于平移的正交矩不变量。
...
for i,image_path1 in tqdm(enumerate(images_path1),total=len(images_path1)):
    ...
    for image_path2 in images_path2:
        ...
        # print(mask_one)
        # 提取特征
        features, labels = pyfeats.zernikes_moments(image, radius=9)
