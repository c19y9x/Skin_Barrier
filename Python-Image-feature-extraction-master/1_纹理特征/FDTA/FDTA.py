# 分形维数纹理分析 (FDTA) 基于分数布朗运动 (FBM) 模型。 
# FBM 模型用于描述自然表面的粗糙度。它将自然出现的表面视为随机游走的最终结果。这种随机游走是我们宇宙中的基本物理过程。
# 表示分形表面的最重要参数之一是分形维数。
# 一种更简单的方法是估计 H 参数（Hurst 系数）。如果在不同分辨率下看到图像，则获得多分辨率分形（MF）特征向量。
...
for i,image_path1 in tqdm(enumerate(images_path1),total=len(images_path1)):
    ...
    for image_path2 in images_path2:
        ...
        # print(mask_one)
        # 提取特征
        features, labels = pyfeats.fdta(image, mask_zero, s=3)
