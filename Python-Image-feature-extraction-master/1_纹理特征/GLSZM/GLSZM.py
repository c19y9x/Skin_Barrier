# 灰度级大小区域矩阵 (GLSZM) 量化图像中的灰度级区域。灰度级区域定义为共享相同灰度级强度的相连体素的数量。
# 如果根据无穷范数（3D 中有 26 个连通区域，2D 中有 8 个连通区域）距离为 1，则认为体素是连通的。 
# GLSZM 特征如下：1) 小区域强调，2) 大区域强调，3) 灰度级不均匀性，4) 区域大小不均匀性，5) 区域百分比，
# 6) 低灰度级区域强调，7 ) 高灰度区域强调, 8) 小区域低灰度强调, 9) 小区域高灰度强调, 10) 大区域低灰度强调, 
# 11) 大区域高灰度强调, 12 灰度方差, 13)区域大小方差，14) 区域大小熵。
...
for i,image_path1 in tqdm(enumerate(images_path1),total=len(images_path1)):
    ...
    for image_path2 in images_path2:
        ...
        # print(mask_one)
        # 提取特征
        features, labels = pyfeats.glszm_features(image, mask_zero)
