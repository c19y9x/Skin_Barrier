# 离散小波变换
# 信号的离散小波变换 (DWT) 被定义为其与一系列函数的内积。对于图像，即二维信号，可以使用二维 DWT。
# 这包括图像行上的 DWT 和结果图像列上的 DWT。每个 DWT 的结果之后分别对列和行进行下采样。
# 图像的分解为每个级别产生四个子图像。每个近似子图像被分解为四个子图像，分别命名为近似子图像、细节水平子图像、细节垂直子图像和细节对角线子图像。
# 每个细节子图像都是与两个半带滤波器卷积的结果。 
# DWT 特征如下：1) 均值和 2) DWT 细节子图像绝对值的标准差。
...
for i,image_path1 in tqdm(enumerate(images_path1),total=len(images_path1)):
    ...
    for image_path2 in images_path2:
        ...
        # print(mask_one)
        # 提取特征
        features, labels = pyfeats.dwt_features(image, mask_one,wavelet='bior3.3', levels=3)
